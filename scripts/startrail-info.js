  ...
  for (const mtrKeys of mtrKeyChunks) {
    const results = await Promise.all(
      mtrKeys.map((key) => mtf
        ...
        .then((details) => ({ key, details })).catch(() => console.warn(`lookup failed for ${key}`)))).
      then(rArr => rArr.filter(r => r !== undefined))
    results.forEach((result) =>
      console.log(
        `\t${result.key}: ${result.details[0] !== ethers.constants.AddressZero
...
const srInfo = async () => {
  ...
  console.log(`contractURI: ${await sr.contractURI().catch(err => {
  ...
  if (hre.network.name === 'polygon') {
    ...
    const metadataDigest = '0x9300d0065e82c9cde4763976f80e50389cedd185cd3bce390411df00f38ab5e5'
    console.log(`tokenURI('${metadataDigest}'): ${await sr['tokenURI(string)'](metadataDigest)}`);
  }
