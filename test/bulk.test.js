...
const {
  randomAddress, randomTokenId, randomBoolean,
...
const generateApproveByCommitmentOrTransferFromWithProvenanceFromSRRs = (tokenIds, withCustomHistoryId) => {
  ...
  const historyMetadataDigest =  randomSha256().toString();
...
const hashTransferFromWithProvenanceSRR = (srr) =>
  ...
  describe("Bulk", () => {
  before(async () => {
    ({ startrailRegistry, bulk, nameRegistry } = await hre.waffle.loadFixture(
      fixtureDefault
    ));
  ...
  describe("generalized bulk", () => {
    ...
    beforeEach(async () => {
      ...
      srrsWithCustomHistoryId = generateApproveByCommitmentOrTransferFromWithProvenanceFromSRRs([
        { functionType: 'approveSRRByCommitment', tokenId: tokenId1WithCustomHistoryId }, 
        { functionType: 'transferFromWithProvenance', tokenId: tokenId2WithCustomHistoryId }, 
      ], true);
      const treeResultWithCustomHistoryId = createMerkleTreeFromSRRs(srrsWithCustomHistoryId);
    ...
    it("success with valid leaf and proof (approveSRRByCommitment)", async () => {
      ...
      const txRspPromise = bulk.approveSRRByCommitmentWithProof(
        ...
        srr.data.tokenId,
        srr.data.commitment,
        srr.data.historyMetadataDigest,
        0
      );
    ...
    it("success with valid leaf and proof (approveSRRByCommitment with customHistoryId)", async () => {
      ...
      await expect(txRspPromise)
        .to.emit(bulk, "ApproveSRRByCommitmentWithProof")
        .withArgs(merkleRootWithCustomHistoryId, srr.data.tokenId, srrLeafString);
    ...
    it("rejects if leaf already processed (transferFromWithProvenance)", async () => {
      ...
      // now call create again for the same leaf
      return expect(transferFromWithProvenanceWithProof(leafIdx)).to.eventually.be.rejectedWith(
        `SRR already processed.`
      );
    });
