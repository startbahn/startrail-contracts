...
```solidity
function verifyLeafHash(bytes32 leafHash, bool isPrimaryIssuer, address artistAddress, bytes32 metadataDigest, string metadataCID, bool lockExternalTransfer) external pure returns (bool hashMatches)
```
...
```solidity
function verifyTransferFromWithProvenanceHash(bytes32 leafHash, address to, uint256 tokenId, string metadataCID, uint256 customHistoryId, bool isIntermediary) external pure returns (bool hashMatches)
```
