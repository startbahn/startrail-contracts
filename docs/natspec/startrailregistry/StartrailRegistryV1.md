...
```solidity
function getSRR(uint256 tokenId) external view returns (struct IStartrailRegistryV1.SRR registryRecord, bytes32 metadataDigest)
```
...
| Name | Type | Description |
|---|---|---|
| registryRecord | IStartrailRegistryV1.SRR | dataset / metadataDigest |
| metadataDigest | bytes32 | undefined |
...
```solidity
event CreateSRR(uint256 indexed tokenId, IStartrailRegistryV1.SRR registryRecord, bytes32 metadataDigest)
```
...
```solidity
event UpdateSRR(uint256 indexed tokenId, IStartrailRegistryV1.SRR registryRecord)
```
