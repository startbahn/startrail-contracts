...
```solidity
function createSRRFromBulk(bool isPrimaryIssuer, address artistAddress, bytes32 metadataDigest, string metadataCID, address issuerAddress, bool lockExternalTransfer_, address royaltyReceiver, uint16 royaltyBasisPoints) external nonpayable returns (uint256)
```
...
```solidity
function createSRRFromLicensedUser(bool isPrimaryIssuer, address artistAddress, bytes32 metadataDigest, string metadataCID, bool lockExternalTransfer_, address to, address royaltyReceiver, uint16 royaltyBasisPoints) external nonpayable
```
...
```solidity
function getSRR(uint256 tokenId) external view returns (struct IStartrailRegistryV23.SRR registryRecord, bytes32 metadataDigest)
```
...
| Name | Type | Description |
|---|---|---|
| registryRecord | IStartrailRegistryV23.SRR | dataset / metadataDigest |
| metadataDigest | bytes32 | undefined |
...
```solidity
event CreateSRR(uint256 indexed tokenId, IStartrailRegistryV23.SRR registryRecord, bytes32 metadataDigest, bool lockExternalTransfer)
```
...
|---|---|---|
| tokenId `indexed` | uint256 | undefined |
| registryRecord  | IStartrailRegistryV23.SRR | undefined |
| metadataDigest  | bytes32 | undefined |
...
```solidity
event RoyaltiesSet(uint256 indexed tokenId, IStartrailRegistryV23.RoyaltyInfo royalty)
```
...
|---|---|---|
| tokenId `indexed` | uint256 | undefined |
| royalty  | IStartrailRegistryV23.RoyaltyInfo | undefined |
...
```solidity
event UpdateSRR(uint256 indexed tokenId, IStartrailRegistryV23.SRR registryRecord)
```
