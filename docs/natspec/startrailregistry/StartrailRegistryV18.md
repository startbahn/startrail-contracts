...
```solidity
function createSRRFromBulk(bool isPrimaryIssuer, address artistAddress, bytes32 metadataDigest, string metadataCID, address issuerAddress, bool lockExternalTransfer_) external nonpayable returns (uint256)
```
...
```solidity
function createSRRFromLicensedUser(bool isPrimaryIssuer, address artistAddress, bytes32 metadataDigest, string metadataCID, bool lockExternalTransfer_, address to) external nonpayable
```
...
| Name | Type | Description |
|---|---|---|
| registryRecord | IStartrailRegistryV18.SRR | dataset / metadataDigest |
| metadataDigest | bytes32 | undefined |
