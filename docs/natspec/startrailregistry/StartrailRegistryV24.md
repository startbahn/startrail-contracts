...
```solidity
function createSRRFromBulk(bool isPrimaryIssuer, address artistAddress, string metadataCID, address issuerAddress, bool lockExternalTransfer_, address royaltyReceiver, uint16 royaltyBasisPoints) external nonpayable returns (uint256)
```
...
*Creates a registryRecord of an artwork from LicensedUser wallet and transfers it to the recipient (deprecated form)*
...
| Name | Type | Description |
|---|---|---|
| registryRecord | IStartrailRegistryV24.SRR | dataset |
