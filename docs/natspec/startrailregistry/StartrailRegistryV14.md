...
```solidity
function getSRR(uint256 tokenId) external view returns (struct IStartrailRegistryV14.SRR registryRecord, bytes32 metadataDigest)
```
...
| Name | Type | Description |
|---|---|---|
| registryRecord | IStartrailRegistryV14.SRR | dataset / metadataDigest |
| metadataDigest | bytes32 | undefined |
...
```solidity
function updateCustomHistory(uint256 customHistoryId, bytes32 metadataDigest) external nonpayable
```
...
```solidity
event CreateSRR(uint256 indexed tokenId, IStartrailRegistryV14.SRR registryRecord, bytes32 metadataDigest)
```
...
|---|---|---|
| tokenId `indexed` | uint256 | undefined |
| registryRecord  | IStartrailRegistryV14.SRR | undefined |
| metadataDigest  | bytes32 | undefined |
...
```solidity
event UpdateCustomHistory(uint256 indexed id, bytes32 metadataDigest)
```
...
```solidity
event UpdateSRR(uint256 indexed tokenId, IStartrailRegistryV14.SRR registryRecord)
```
