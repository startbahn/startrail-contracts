...
```solidity
function createSRRFromBulk(bool isPrimaryIssuer, address artistAddress, bytes32 metadataDigest, string metadataCID, address issuerAddress, bool lockExternalTransfer_, address royaltyReceiver, uint16 royaltyPercentage) external nonpayable returns (uint256)
```
...
*Creates a registryRecord of an artwork from LicensedUser wallet*
...
```solidity
function createSRRFromLicensedUser(bool isPrimaryIssuer, address artistAddress, bytes32 metadataDigest, string metadataCID, bool lockExternalTransfer_, address to, address royaltyReceiver, uint16 royaltyPercentage) external nonpayable
```
...
| isPrimaryIssuer | bool | address of the issuer user contract |
| artistAddress | address | address of the artist contract |
| metadataDigest | bytes32 | bytes32 of metadata hash used by detect if createSRR is duplicate |
...
```solidity
function getSRR(uint256 tokenId) external view returns (struct IStartrailRegistryV20.SRR registryRecord, bytes32 metadataDigest)
```
...
| Name | Type | Description |
|---|---|---|
| registryRecord | IStartrailRegistryV20.SRR | dataset / metadataDigest |
| metadataDigest | bytes32 | undefined |
...
```solidity
function royalties(uint256) external view returns (address receiver, uint16 percentage)
```

Royalties marked `public` to provide direct read access to the percentages. required for proposed SBINFT implementation AND it&#39;s handy to have this lookup as royaltyInfo doesn&#39;t show this. although it could be derived: `royaltyInfo(tokenId, 1 ether);
...
```solidity
event CreateSRR(uint256 indexed tokenId, IStartrailRegistryV20.SRR registryRecord, bytes32 metadataDigest, bool lockExternalTransfer)
```
...
|---|---|---|
| tokenId `indexed` | uint256 | undefined |
| registryRecord  | IStartrailRegistryV20.SRR | undefined |
| metadataDigest  | bytes32 | undefined |
...
```solidity
event RoyaltiesSet(uint256 indexed tokenId, IStartrailRegistryV20.RoyaltyInfo royalty)
```
...
|---|---|---|
| tokenId `indexed` | uint256 | undefined |
| royalty  | IStartrailRegistryV20.RoyaltyInfo | undefined |
...
```solidity
event UpdateSRR(uint256 indexed tokenId, IStartrailRegistryV20.SRR registryRecord)
```
