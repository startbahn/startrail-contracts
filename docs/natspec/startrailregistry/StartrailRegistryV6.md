...
*Creates a registryRecord of an artwork This method exists for backward compatibility. We plan to remove it in a future release.*
...
```solidity
function createSRRFromLicensedUser(bool isPrimaryIssuer, address artistAddress, bytes32 metadataDigest) external nonpayable
```



*Creates a registryRecord of an artwork from LicensedUserLogic contract This method exists for backward compatibility. We plan to remove it in a future release.*
...
```solidity
function getSRR(uint256 tokenId) external view returns (struct IStartrailRegistryV6.SRR registryRecord, bytes32 metadataDigest)
```
...
| Name | Type | Description |
|---|---|---|
| registryRecord | IStartrailRegistryV6.SRR | dataset / metadataDigest |
| metadataDigest | bytes32 | undefined |
...
```solidity
event CreateSRR(uint256 indexed tokenId, IStartrailRegistryV6.SRR registryRecord, bytes32 metadataDigest, bool lockExternalTransfer)
```
...
|---|---|---|
| tokenId `indexed` | uint256 | undefined |
| registryRecord  | IStartrailRegistryV6.SRR | undefined |
| metadataDigest  | bytes32 | undefined |
...
```solidity
event UpdateSRR(uint256 indexed tokenId, IStartrailRegistryV6.SRR registryRecord)
```
