 ...
 */
contract MetaTxRequestManager is Initializable, IMetaTxRequest {
  //
  // Types
  //
    ...
    onlyAdministrator
  {
    ...
    string memory requestType = string(
      abi.encodePacked(_typeName, "(", GENERIC_PARAMS, ",", _typeSuffix, ')')
    );
    ...
    returns (bytes memory)
  {
    ...
    bytes memory encodedStructSuffix = (_request.callData.length > 0) ?
      abi.encodePacked(keccak256(_request.callData), _request.suffixData) :
    ...
    //
    return
      abi.encodePacked(bytes1(0x19), bytes1(0x01), domainSeparator, txHash);
  }
}
