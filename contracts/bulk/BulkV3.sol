...
contract BulkV3 is Contracts, InitializableWithGap, EIP2771BaseRecipient {
     ...
     * @param lockExternalTransfers list of lock transfer flags
     * @param royaltyReceivers list of royalty receivers
     * @param royaltyPercentages list of royalty percentage
     */
    function createSRRWithProofMulti(
        bytes32[][] memory merkleProofs,
        bytes32 merkleRoot,
        ...
        uint16[] memory royaltyPercentages
    ) public batchExists(merkleRoot) returns (uint256[] memory tokenIds) {
        tokenIds = new uint256[](leafHashes.length);
        ...
        for (uint256 i = 0; i < leafHashes.length; i++) {
            ...
            tokenIds[i] = _createSRRWithProof(
                ...
                royaltyPercentages.length != 0 ? royaltyPercentages[i] : 0
            );
        }
    }
