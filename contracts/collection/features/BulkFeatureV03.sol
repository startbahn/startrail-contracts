...
import "./interfaces/IBulkFeatureV03.sol";
 ...
 */
contract BulkFeatureV03 is IBulkFeatureV03, Contracts {
    ...
    ) public override(IBulkFeatureV03) returns (uint256) {
        onlyBulk();
    ...
    ) public override(IBulkFeatureV03) {
        onlyBulk();
    ...
    function transferFromWithProvenanceFromBulk(
        ...
        uint256 customHistoryId,
        bool isIntermediary
    ) external override(IBulkFeatureV03) {
        onlyBulk();
        LibFeatureCommonV02.onlyCollectionOwnerOrSRROwner(signer, tokenId);
