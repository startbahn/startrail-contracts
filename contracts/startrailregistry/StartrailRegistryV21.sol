...
import "../common/IStartrailRegistryV21.sol";
...
contract StartrailRegistryV21 is
    Contracts,
    IStartrailRegistryV21,
    ...
    IERC2981
{
    ...
    ) public override(IStartrailRegistryV21) trustedForwarderOnly {
        _createSRR(
            isPrimaryIssuer,
            artistAddress,
            metadataDigest,
    ...
    function createSRRFromBulk(
        ...
        bool lockExternalTransfer_
    ) public override(IStartrailRegistryV21) returns (uint256) {
        onlyBulk();
        return
            _createSRR(
                isPrimaryIssuer,
    ...
    ) public override(IStartrailRegistryV21) {
        onlyBulk();
    ...
    function transferFromWithProvenanceFromBulk(
        ...
        uint256 customHistoryId,
        bool isIntermediary
    ) external override(IStartrailRegistryV21) {
        onlyBulk();
    ...
    function updateSRRMetadata(uint256 tokenId, bytes32 metadataDigest)
        external
        override(IStartrailRegistryV21)
    {
        tokenExists(tokenId);
        onlyIssuerOrArtistOrAdministrator(tokenId);
