...
import "../common/IStartrailRegistryV26.sol";
import "../lib/IDGeneratorV3.sol";
import "../common/EIP2771Or4337Recipient.sol";
...
contract StartrailRegistryV26 is
    Contracts,
    IStartrailRegistryV26,
    IStartrailRegistryMigrationV2,
    InitializableWithGap,
    Storage,
    ERC721UpgradeSafe,
    EIP2771Or4337Recipient,
    IERC2981
{
    /*
     * Constants
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(IERC165, ERC721UpgradeSafe) returns (bool) {
        return
            interfaceId == type(IERC2981).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    function licensedUserManager() private view returns (ILicensedUserManager) {
        return
    ...
    function onlyLicensedUserOrAdministrator() private view {
        if (!isAdministrator()) {
            require(
                licensedUserManager().isActiveWallet(msgSender()),
    ...
    function onlyIssuerOrAdministrator(uint256 tokenId) private view {
        ...
        address aliasBackward = licensedUserManager()
            .walletAddressAliasBackward(sender);
        require(
            sender == _addressStorage[tokenId][_SRR][_ISSUER] ||
                (aliasBackward != address(0x0) &&
                    aliasBackward == _addressStorage[tokenId][_SRR][_ISSUER]) ||
                isAdministrator(),
     ...
     * @dev Reverts with "This is not a SRR owner" error message if the toCheck is not the owner.
     */
    function onlySRROwner(address toCheck, uint256 tokenId) private view {
        ...
        require(
            tokenOwner == toCheck ||
                (aliasBackward != address(0x0) && tokenOwner == aliasBackward),
            "This is not a SRR owner"
        );
    }
    ...
    function createSRRFromLicensedUser(
        ...
        public
        override(IStartrailRegistryV26)
        trustedForwarderAndDeployedActiveWalletOnly(
            INameRegistry(nameRegistryAddress).get(
                Contracts.LICENSED_USER_MANAGER
            )
        )
    ...
    function createSRRFromBulk(
        ...
        address royaltyReceiver,
        uint16 royaltyBasisPoints
    ) public override(IStartrailRegistryV26) returns (uint256) {
        onlyBulk();
        uint256 tokenId = _createSRR(
            isPrimaryIssuer,
            artistAddress,
        ...
        if (to != address(0)) {
            ERC721UpgradeSafe._transfer(issuerAddress, to, tokenId);
        }
        return tokenId;
    }
    ...
    ) public override(IStartrailRegistryV26) {
        onlyBulk();
    ...
    function transferFromWithProvenanceFromBulk(
        ...
        uint256 customHistoryId,
        bool isIntermediary
    ) external override(IStartrailRegistryV26) {
        onlyBulk();
        onlySRROwner(signer, tokenId);
    ...
    function updateSRRMetadata(
        ...
        emit UpdateSRRMetadataCID(tokenId, metadataCID);
    }
    ...
    function updateSRRRoyalty(
        ...
        if (royalty.receiver != address(0)) {
            ...
            emit RoyaltiesSet(
                tokenId,
                RoyaltyInfo(royaltyReceiver, royaltyBasisPoints)
            );
        }
    }
    ...
    function updateSRRRoyaltyReceiverMulti(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (royalty.receiver != address(0)) {
                ...
                emit RoyaltiesSet(
                    tokenIds[i],
                    RoyaltyInfo(royaltyReceiver, royalty.basisPoints)
                );
            }
        }
    }
     ...
     * @param nameRegistry address of the NameRegistry
     */
    function setNameRegistryAddress(
     ...
     * @return metadataDigest srr metadata digest
     * @return metadataCID srr metadata cid
     */
    function getSRR(
        uint256 tokenId
    )
        ...
        )
    {
        ...
        metadataCID = _stringStorage[tokenId][_SRR][_METADATA_HASH];
    }
    ...
    ) public view override(IStartrailRegistryV26) returns (string memory) {
     ...
     */
    function addCustomHistoryType(
        string memory historyTypeName
    ) external override(IStartrailRegistryV26) {
        onlyAdministrator();
        require(
    ...
    function _createSRR(
        ...
        require(isNotEmptyString(metadataCID), "SRR metadataCID is required");
     ...
     * @param tokenId uint256 of StartrailRegistryRecordID
     */
    function lockExternalTransfer(
        uint256 tokenId
    ) public view override(IStartrailRegistryV26) returns (bool) {
