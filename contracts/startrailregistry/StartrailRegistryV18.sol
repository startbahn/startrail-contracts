...
import "../common/IStartrailRegistryV18.sol";
...
contract StartrailRegistryV18 is
    Contracts,
    IStartrailRegistryV18,
    ...
    EIP2771BaseRecipient
{
    ...
    ) public override(IStartrailRegistryV18) trustedForwarderOnly {
        _createSRR(
            isPrimaryIssuer,
            artistAddress,
            metadataDigest,
    ...
    function createSRRFromBulk(
        ...
        bool lockExternalTransfer_
    ) public override(IStartrailRegistryV18) returns (uint256) {
        onlyBulk();
        return
            _createSRR(
                isPrimaryIssuer,
    ...
    ) public override(IStartrailRegistryV18) {
        onlyBulk();
    ...
    function transferFromWithProvenanceFromBulk(
        ...
        uint256 customHistoryId,
        bool isIntermediary
    ) external override(IStartrailRegistryV18) {
        onlyBulk();
    ...
    function updateSRRMetadata(uint256 tokenId, bytes32 metadataDigest)
        external
        override(IStartrailRegistryV18)
    {
        tokenExists(tokenId);
        onlyIssuerOrArtistOrAdministrator(tokenId);
        ...
        )
    {
        ...
        historyMetadataHash = _stringStorage[tokenId][_HISTORY][
            _METADATA_HASH
        ];
    ...
    {
        bool isCID = isNotEmptyString(_stringStorage[tokenId][_SRR][_METADATA_HASH]);
        if(isCID) {
            return string(abi.encodePacked("https://", _stringStorage[tokenId][_SRR][_METADATA_HASH], ".ipfs.nftstorage.link"));
        } else {
            return
    ...
    function updateCustomHistory(
        ...
        if (keccak256(bytes(getCustomHistoryNameById(customHistoryId))) != keccak256(bytes(name))) {
            _stringStorage[customHistoryId][_CUSTOM_HISTORY][_CUSTOM_HISTORY_NAME] = name;
        }
    ...
    function _createSRR(
        ...
        if (isCID) {
            ...
            _bytes32Storage[tokenId][_SRR][_METADATA_HASH] = metadataDigestBytes32;
        }
        ...
        if (isCID) {
            emit CreateSRR(tokenId, SRR(isPrimaryIssuer, artistAddress, sender), metadataCID, lockExternalTransfer_);
        } else {
            emit CreateSRR(tokenId, SRR(isPrimaryIssuer, artistAddress, sender), metadataDigestBytes32, lockExternalTransfer_);
        }
    }
    ...
    function isNotEmptyString(string memory _string) internal pure returns (bool) {
