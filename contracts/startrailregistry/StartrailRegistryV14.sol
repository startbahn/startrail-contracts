...
import "../common/IStartrailRegistryV14.sol";
...
contract StartrailRegistryV14 is
    Contracts,
    ...
    EIP2771BaseRecipient
{
    ...
    ) public override(IStartrailRegistryV14) trustedForwarderOnly {
        _createSRR(
            isPrimaryIssuer,
            artistAddress,
            metadataDigest,
    ...
    function createSRRFromBulk(
        ...
        bool lockExternalTransfer_
    ) public override(IStartrailRegistryV14) returns (uint256) {
        onlyBulk();
        return
            _createSRR(
                isPrimaryIssuer,
    ...
    ) public override(IStartrailRegistryV14) {
        onlyBulk();
    ...
    function transferFromWithProvenanceFromBulk(
        ...
        uint256 customHistoryId,
        bool isIntermediary
    ) external override(IStartrailRegistryV14) {
        onlyBulk();
    ...
    function updateSRRMetadata(uint256 tokenId, bytes32 metadataDigest)
        external
        override(IStartrailRegistryV14)
    {
        onlyLicensedUserOrAdministrator();
        tokenExists(tokenId);
