...
import "../common/IStartrailRegistryV16.sol";
...
contract StartrailRegistryV16 is
    Contracts,
    ...
    EIP2771BaseRecipient
{
    ...
    ) public override(IStartrailRegistryV16) trustedForwarderOnly {
        _createSRR(
            isPrimaryIssuer,
            artistAddress,
            metadataDigest,
    ...
    function createSRRFromBulk(
        ...
        bool lockExternalTransfer_
    ) public override(IStartrailRegistryV16) returns (uint256) {
        onlyBulk();
        return
            _createSRR(
                isPrimaryIssuer,
    ...
    ) public override(IStartrailRegistryV16) {
        onlyBulk();
    ...
    function transferFromWithProvenanceFromBulk(
        ...
        uint256 customHistoryId,
        bool isIntermediary
    ) external override(IStartrailRegistryV16) {
        onlyBulk();
    ...
    function updateSRRMetadata(uint256 tokenId, bytes32 metadataDigest)
        external
        override(IStartrailRegistryV16)
    {
        tokenExists(tokenId);
        onlyIssuerOrArtistOrAdministrator(tokenId);
