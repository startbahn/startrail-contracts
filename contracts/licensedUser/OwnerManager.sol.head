 ...
 *  - use EIP2771 to receive calls from a Forwarder contract OR directly by an active deployed LicensedUserWallet
 */
contract OwnerManagerV02 is InitializableWithGap, EIP2771Or4337Recipient {
    ...
    /**
     * @dev Determine if the call is from either forwarder with a the given LicensedUser EIP2771 forwarded,
     *  or directly by an active deployed LicensedUserWallet.
     *
     * If it's from forwarder, the given wallet address must be the last bytes of the call data (an EIP2771 forward).
     *
     * @return "" if valid call with _wallet or "<specific error message>" if not
     *   a valid call with _wallet
     */
    function isFromWallet(
        address _wallet
    ) internal view returns (string memory) {
        bool isFromForwarder = isTrustedForwarder(msg.sender);
        bool isFromActiveDeployedWallet = isActiveDeployedWallet(
            msg.sender,
            address(this)
        );
        if (!isFromForwarder && !isFromActiveDeployedWallet) {
            return
                "Wallet function can only be called from the trusted forwarder or directly from an active deployed wallet";
        }

        if (isFromForwarder) {
            if (msg.data.length < 24) {
                return
                    "Wallet address must be appended to calldata (EIP2771 forward)";
            }
    ...
    function addOwner(
        ...
        address _owner,
        uint8 _threshold
    ) public virtual onlyWalletOrAdministrator(_wallet) {
        // Owner address cannot be null.
        require(
    ...
    function getThreshold(address _wallet) public view virtual returns (uint8) {
    ...
    /**
     * @dev Deletes a wallet's ownership state by clearing the owner linked list
     *      and resetting metadata. Intended for internal migration flows.
     */
    function _deleteWallet(address _wallet) internal {
        Wallet storage wallet = wallets[_wallet];
        if (wallet.ownerCount == 0) {
            delete wallets[_wallet];
            return;
        }

        address[] memory owners = getOwners(_wallet);
        for (uint256 i = 0; i < owners.length; i++) {
            delete wallet.owners[owners[i]];
        }
        delete wallet.owners[SENTINEL_OWNERS];
        delete wallets[_wallet];
    }
