...
import "./proxy/WalletProxyMinimal.sol";
import "./OwnerManager.sol";
import "./SignatureChecker.sol";

/**
 * @title LicensedUserManager - manages LicensedUser wallets.
 ...
 */
contract LicensedUserManager is 
  SignatureChecker,
  OwnerManager
{
  //
  ...
  // Hash of the WalletProxyMinimal bytecode for use with create2
  bytes32 public constant PROXY_CODEHASH = keccak256(
    type(WalletProxyMinimal).creationCode
  );
  ...
  modifier isValidCreateWalletRequest(
    ...
    string calldata _englishName,
    string calldata _originalName
  ) {
    require(walletExists(_walletAddress) == false, "A wallet already exists for this address");
    require(isEmptyString(_englishName) == false, "englishName must not be empty");
    require(isEmptyString(_originalName) == false, "originalName must not be empty");
    _;
  }
  ...
  {
    walletAddress = Create2.computeAddress(_salt, PROXY_CODEHASH);
